...
def gerenate_random_vectors(x_max,y_max,z_max,radius,min_groups,max_groups):
	...
	for i in range(0,groups_num):
		point_c = np.array([random.uniform(0,x_max), random.uniform(0,y_max), random.uniform(0,z_max)])
		point_num = random.randint(20,80)

		for j in range(0,point_num):
			point = Point(point_c + np.array([random.uniform(-radius,radius), random.uniform(-radius,radius), random.uniform(-radius,radius)]))
			points.append(point)

	print("Random Vector Function Complete. {} numbers of points was created in {} groups.".format(len(points),groups_num))
	return points
#===========================================================
	...
	for i in range(1,k):
		...
		for index in random_index:
			...
			for rep_v in rep_vs:
				...
				corel += abs(np.dot(point_dm,rep_v_dm)/(point_dm_norm*rep_v_dm_norm))
			if corel > corel_max:
				max_index = index
				corel_max = corel
		rep_vs.append(points[max_index].coordinates)
