...
def plot_osbtacles(obstacles, axes):
	for obstacle in obstacles:
		circle = Circle((obstacle.center[0],obstacle.center[1]), obstacle.radius, edgecolor='red', facecolor='none')
...
def create_diagonal_obstacles(obstacle_number, start, finish, min_size, max_size, obstacles):
	...
	while count < obstacle_number :
		jump = random.uniform(step*.5, step*1.5)
		...
		if calculate_distance(start,new_obstacle.center)<=new_obstacle.radius*1.1:
					continue
		elif calculate_distance(finish,new_obstacle.center)<=new_obstacle.radius*1.1:
					continue
		...
		if count==0:
			...
			for obstacle in obstacles:
				if(calculate_distance(obstacle.center,new_obstacle.center))<(new_obstacle.radius+obstacle.radius)*1.2:
...
def create_peripheral_obstacles(obstacle_number, start, finish, min_size, max_size, obstacles):
	count=0
	while count < obstacle_number:
...
def calculate_next_step(start, finish, step, obstacles):
	...
	next_step, next_step_x, next_step_y = calculate_next_destination(start,finish,step)
	...
	for obstacle in obstacles:
		...
		if calculate_distance(next_step,obstacle.center)<=obstacle.radius or (d<obstacle.radius*1.5 and abs(dist_1+dist_2-d_3)<0.1):
			tang_1, tang_2 = find_tangent(start, obstacle)
			tang = choose_direction(tang_1,tang_2,finish)
			next_step, next_step_x, next_step_y = calculate_next_destination(start,tang,step)
...
def path_planning(start, finish, step,obstacles):
	...
	while calculate_distance(current,finish)>step/2:
		current, next_step_x, next_step_y=calculate_next_step(current,finish,step,obstacles)
...
obstacles = create_peripheral_obstacles(30,start,finish,min_size/3,max_size,obstacles)
