...
def plot_osbtacles(obstacles, axes):
	for obstacle in obstacles:
		circle = Circle(obstacle.center, obstacle.radius, edgecolor='red', facecolor='none')
...
def plot_path(paths, start, end, obstacles, axes):
	...
	return
#===========================================================
def create_diagonal_obstacles(obstacle_number, start, end, min_size, max_size, obstacles):
	count=0
	...
	while count < obstacle_number:
		...
		next_position = start + random.uniform(step*.5, step*1.5) * direction_aligned
		obstacle_center = next_position + random.uniform(-step*.5,step*.5)*diretion_perpendicular
		...
		if count==0:
			...
			for obstacle in obstacles:
				if(np.linalg.norm(obstacle.center-new_obstacle.center))<(new_obstacle.radius+obstacle.radius)*1.2:
	...
	return obstacles
#===========================================================
def create_peripheral_obstacles(obstacle_number, start, end, min_size, max_size, obstacles):
	count=0
	while count < obstacle_number:
...
#===========================================================
def create_random_obstacles(diag_obstacle_number, periph_obstacle_number, start, end, min_size, max_size,per_obs_coef):
	obstacles=[]

	obstacles = create_diagonal_obstacles(diag_obstacle_number,start,end,min_size,max_size,obstacles)
	obstacles = create_peripheral_obstacles(periph_obstacle_number,start,end,min_size*per_obs_coef,max_size*per_obs_coef,obstacles)
...
def line_circle_intersection(start,end,obstacle):
	...
	if dist_center_end<obstacle.radius or (dist_closest_center<obstacle.radius and dist_closest_start+dist_closest_end - dist_start_end <0.0):
		return True
	else:
		return False
#===========================================================
def find_tangent(point, obstacle):
    ...
    tangent_position1 = obstacle.center + obstacle.radius * np.array([np.cos(tangent_angle1), np.sin(tangent_angle1)])
    tangent_position2 = obstacle.center + obstacle.radius * np.array([np.cos(tangent_angle2), np.sin(tangent_angle2)])
...
def calculate_next_step(start, end, step, obstacles,direction):
	...
	for obstacle in obstacles:
		if line_circle_intersection(start,next_step,obstacle)==True:
			...
			tang_1, tang_2 = choose_direction(tang_1,tang_2,end,direction,"bidirectional")
			next_step_1 = start + step*(tang_1-start)/np.linalg.norm(tang_1-start)
			next_step_2 = start + step*(tang_2-start)/np.linalg.norm(tang_2-start)
...
def path_planning(path, paths, start, end, step, obstacles):
	...
	while np.linalg.norm(current-end)>step/2:
		current, encounter, obstacle, next_option = calculate_next_step(current,end,step,obstacles,path.direction)
		if encounter==True and obstacle not in path.obstacles:
			...
			new_path=Path(alternative_path,new_path_obstacles,False,not path.direction)
